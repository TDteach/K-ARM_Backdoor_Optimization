...
class K_Arm_Scanner:
    ...
    def update_tensor(self, mask_tanh_tensor, pattern_tanh_tensor, y_target_index, first=False):
        ...
        if first is True:
            ...
            self.pattern_raw_tensor[y_target_index] = (
                        torch.tanh(pattern_tanh_tensor[y_target_index]) / (2 - self.epsilon) + 0.5)

    def scanning(self, target_classes_all, data_loader_arr, y_target_index, pattern_init, mask_init, trigger_type,
                 direction, preprocess_func=None):
        ...
        for i in range(self.num_classes):
            optimizer = optim.Adam([self.pattern_tanh_tensor[i], self.mask_tanh_tensor[i]], lr=self.lr,
                                   betas=(0.5, 0.9))
        ...
        for step in pbar:
            ...
            loss_list = []
            loss_acc_list = []
            for idx, data in enumerate(data_loader_arr[y_target_index]):
                img, label = data[0], data[-1]
                img = img.to(self.device)
                Y_target = y_target_tensor.repeat(img.size()[0])

                X_adv_tensor = (1 - self.mask_tensor[y_target_index]) * img + self.mask_tensor[y_target_index] * \
                               self.pattern_raw_tensor[y_target_index]

                if preprocess_func is not None:
                    X_adv_tensor = preprocess_func(X_adv_tensor)
                ...
                pbar.set_description(
                    'Target: {}, victim: {}, Loss: {:.4f}, Acc: {:.2f}%, CE_Loss: {:.2f}, Reg_Loss:{:.2f}, Cost:{:.2f} best_reg:{:.2f} avg_loss_reg:{:.2f}'.format(
                        target_classes_all[y_target_index], label[0], self.loss, self.loss_acc * 100, self.loss_ce,
                        self.loss_reg, self.cost_tensor[y_target_index], best_reg[y_target_index],
                        avg_loss_reg[y_target_index]))
            ...
            if direction == 'forward':
                ...
                if (total_times[y_target_index] > 20 and best_acc[
                    y_target_index] < 0.3 and trigger_type == 'polygon_specific') or (
                        total_times[y_target_index] > 200 and best_acc[
                    y_target_index] < 0.8 and trigger_type == 'polygon_specific') or (
                        total_times[y_target_index] > 10 and best_acc[
                    y_target_index] == 0 and trigger_type == 'polygon_specific'):
                    early_stop_tag[y_target_index] = True

            elif direction == 'backward':
                if (total_times[y_target_index] > 200 and best_acc[
                    y_target_index] < 1 and trigger_type == 'polygon_specific'):
                    # for the backward check
                    early_stop_tag[y_target_index] = True

            update[y_target_index] = False
            if avg_loss_acc[y_target_index] >= self.attack_succ_threshold and avg_loss_reg[y_target_index] < best_reg[
                y_target_index]:
                ...
                reg_down_vel[y_target_index] = ((first_best_reg[y_target_index]) - (avg_loss_reg[y_target_index])) / (
                            times[y_target_index] + (total_times[y_target_index] / 2))
            ...
            if self.early_stop:
                ...
                if (times[y_target_index] > self.round) or (
                        cost_down_flag[y_target_index] and cost_up_flag[y_target_index] and early_stop_counter[
                    y_target_index] > self.early_stop_patience and trigger_type == 'polygon_global'):
            ...
            if early_stop_tag[y_target_index] == True and len(non_opt_index) != 0:
                ...
                if update[y_target_index] and times[y_target_index] >= self.warmup_rounds and all(
                        ele >= self.warmup_rounds for ele in time_tensor):
                    ...
                    if random_value < self.epsilon_for_bandits:
                        ...
                        if len(non_early_stop_index) > 1:
                            y_target_index = non_early_stop_index[
